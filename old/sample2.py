...
def display():
    ...
    if gameState > 0 and gameState < 3:
        glPushMatrix()
        ...
        for i in range(len(objPos)):
            if objPos[i][1] < ypos - 5:
                continue
            glTranslatef(objPos[i][0], objPos[i][1], 2.0)
...
def set_texture():
    ...
    # imgName = "index.png"
    name = ["green.png", "block.png", "grayblock.png"]
    # global g_TextureHandlesVideo
    
    global gTextureHandles
    gTextureHandles = glGenTextures(3)
